...
public class orangeGhostController : MonoBehaviour
{
    ...
    */
    ...
    /* Change back after debug testing
    private static int orangePelletCount = 90;
    */
    public static int orangePelletCount = 20;
    ...
    void Start()
    {
        ...
        currentCount = orangePelletCount;
        speed = 0.0f;
        orangeTarget.transform.position = new Vector2(10.5f,5f);
    }
    ...
    private void intersection()
    {
        ...
        {
            targetX = orangeTarget.transform.position.x;
            targetY = orangeTarget.transform.position.y;
    ...
    private void targetManagement()
    {
        if (mode == 2)
        {
            altX = player.transform.position.x - altX;
            altY = player.transform.position.y - altY;
            totalDistance = Mathf.Sqrt(altX * altX + altY * altY);
            if (totalDistance >= 8.0f)
            {
                orangeTarget.transform.position = new Vector2(player.transform.position.x,player.transform.position.y);
            }
            else
            {
                orangeTarget.transform.position = new Vector2(18.3f,-13f);
            }
        }
        if (mode == 1)
        {
            // This vector 2 needs to be updated to roughly whatever the bottom left corner is.
            orangeTarget.transform.position = new Vector2(-17.5f,-13f);
        }
    }
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.tag == "orangeTarget")
        {
            if (mode == 3 && starting == false)
            {
                ...
                {
                    orangeTarget.transform.position = new Vector2(10.5f,3.0f);
                    face = 2;
                    animator.SetInteger("face", face);
                    direction = Vector2.down;
                    targetCollision += 1;
                ...
                if (targetCollision == 1)
                {
                    ...
                    animator.SetInteger("face", face);
                    direction = Vector2.up;
                    orangeTarget.transform.position = new Vector2(10.5f,5.0f);
                    targetCollision += 1;
                }
            }
            if (starting)
